...
export class ChatComponent {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.pubnub.init({
      ...
      ssl: environment.SSL,
      logVerbosity: environment.LOG_VERBOSITY,
      uuid: environment.UUID,
      apns2Env: environment.APP_ENV,
      apns2BundleId: environment.APP_BUNDLE_ID,
    });
    if(token) {
  ...
  async getToken() {
    ...
    return this.Http.post('http://localhost:3001/auth/token', { code: access_token, clientId: "temp_client" }).subscribe(res => {
  ...
  async sendMessage(event: { message: string }, userName: string, avatar: string) {
    if (!this.inRoom) {
      return;
    }
